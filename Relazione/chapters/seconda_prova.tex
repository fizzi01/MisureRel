...
Il filtro RC è un sistema che effettua sul segnale di ingresso una funzione di attenuazione in quanto filtro passivo, cioè composto da componenti passivi quali un condensatore e un resistore in serie.
...
\begin{itemize}
    ...
    \'E importante osservare che l'ampiezza visualizzata sull'oscilloscopio è diversa da quella impostata sul generatore di funzioni a causa del disadattamento di impedenza tra i due strumenti. In particolare, in corrispondenza della tensione picco-picco di 5 V impostata sul generatore di funzioni per l'ingresso del filtro, verrà visualizzata una tensione picco-picco di 10 V sul display dell'oscilloscopio.
    ...
    \begin{enumerate}
        ...
        \item Impostazione del \emph{Coupling} su DC mediante il secondo pulsante posizionato sotto lo schermo  
    \end{enumerate}
\end{itemize}
...
\[U_f = 338,63 Hz\cdot U_{r,f} = 338,63Hz \cdot 20 \cdot 10^{-6} = 6,8\cdot10^{-3}Hz\]
